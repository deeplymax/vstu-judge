...
const ProblemDatabaseConnectionOptions = {
  "url": "mongodb://vstu-judge-problem-db:27017/vstu_judge",
  "options": {}
};
...
function getContainerURL(connectionOptions, containerData) {
  ...
  Logger.error(`ASSASA ${containerURL}\n`);
  return containerURL;
}
    ...
    function runNextTest(submissionArtifact, tests) {
      ...
      Docker.run(testImage, command, streams, options, error => {
        if (error) {
          processError({
            "error": error,
            "message": `1111111Failed to run the test container '${testImage}' ` +
...
Server.listen(ServerPort, () => {
  Logger.info(`vstu-judge-back is listening on port ${ServerPort}.`);
});
