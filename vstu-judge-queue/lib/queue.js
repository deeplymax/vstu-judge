...
function Queue() {
  ...
  let configurationFiles = [
    "./vstu-judge-queue-configuration.json"
  ];
  this.configuration =
...
Queue.prototype = {
  ...
  _resolveConfiguration: function(configuration) {
    ...
    configuration["backServer"] =
      process.env["VSTU_JUDGE_QUEUE_BACK_SERVER"] ||
        configuration["backServer"] || "http://vstu-judge-back:7070";
    ...
    databaseConfigurations["service"] =
      this._extractEnvironmentConfiguration("VSTU_JUDGE_QUEUE_DISCOVERY_DATABASE") ||
        databaseConfigurations["service"];
    databaseConfigurations["queue"] =
      this._extractEnvironmentConfiguration("VSTU_JUDGE_QUEUE_QUEUE_DATABASE") ||
        databaseConfigurations["queue"] || {
          "host": "vstu-judge-queue-db",
          "port": "6379"
        };
    databaseConfigurations["task"] =
      this._extractEnvironmentConfiguration("VSTU_JUDGE_QUEUE_TASK_DATABASE") ||
        databaseConfigurations["task"] || {
          "url": "mongodb://vstu-judge-task-db:27017/vstu_judge",
          "options": { }
        };
